...
extension NSPersistentContainer {
    ...
    /// - Parameter newURL: A file URL that specifies a *new* location for the store. If not specified, will keep in same location.
    /// - Throws: `CopyPersistentStoreError` in various situations.
    /// - Returns: Nothing. If no errors are thrown, the restore is complete.
    func restorePersistentStore(from backupURL: URL, migrateTo newURL: URL? = nil) throws -> Void {
        ...
        for persistentStore in persistentStoreCoordinator.persistentStores {
            ...
            do {
                ...
                let restoredTemporaryStore = try persistentStoreCoordinator.migratePersistentStore(backupStore, to: newURL ?? loadedStoreURL, options: persistentStore.options, withType: persistentStore.type)
