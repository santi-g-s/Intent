...
final class HabitTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        dataManager = DataManager.preview //DataManager.testing
    }
    ...
    func test_Score_Super_Long() {
        ...
        let habit = Habit(context: dataManager.container.viewContext)
        habit.startDate_ = Date().addingTimeInterval(-TimeInterval(60*60*24*(9)))
        
        var expected: Double = 0.0
        
        for i in stride(from: 999, through: 0, by: -1) {
